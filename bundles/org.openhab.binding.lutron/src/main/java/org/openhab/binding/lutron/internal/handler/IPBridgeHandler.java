...
public class IPBridgeHandler extends BaseBridgeHandler {
    ...
    private synchronized void connect() {
        ...
        keepAlive = scheduler.scheduleWithFixedDelay(this::sendKeepAlive, heartbeatInterval, heartbeatInterval,
                TimeUnit.MINUTES);
        logger.trace("Keepalive job status: delay {} done {} cancelled {}", keepAlive.getDelay(TimeUnit.SECONDS),
                keepAlive.isDone(), keepAlive.isCancelled());
    }
    ...
    private void parseUpdates() {
        ...
        for (String line : this.session.readLines()) {
            ...
            // logger.trace("Keepalive job status: delay {} done {} cancelled {}", keepAlive.getDelay(TimeUnit.SECONDS),
            // keepAlive.isDone(), keepAlive.isCancelled());
            // logger.trace("Scheduler stat: {} shut: {} term: {}", scheduler.toString(), scheduler.isShutdown(),
            // scheduler.isTerminated()); // For thread pool debugging
